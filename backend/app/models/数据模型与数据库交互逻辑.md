...
存储注册、重置密码等业务的验证码。
- **id** (`bigint_unsigned`): 主键。
- **contact** (`varchar_255`): 联系方式，可存邮箱或手机号。
- **code** (`varchar_10`): 验证码。
- **code_type** (`varchar_20`): 类型: `register`、`reset_password`、`change_email`、`change_phone` 等。
- **expires_at** (`timestamp`): 过期时间。
- **is_used** (`tinyint`): 状态: 0=未使用, 1=已使用。
- **is_deleted** (`tinyint`): 软删除状态: 0=正常, 1=已删除。
- **created_at** (`timestamp`): 创建时间。
...
CREATE TABLE IF NOT EXISTS users (
    ...
    -- 其他字段详见 backend/pkg/db/mysql.go
