...
import type { RealnameStatus, RealnameVerification } from '@/service/api/admin/realname'
...
function getUserDisplayName(userId: number): string {
  ...
  if (!user)
    return t('adminRealname.userFallback', { id: userId })
...
const columns: DataTableColumns<RealnameVerification> = [
  ...
  { title: t('realname.certificateNo'), key: 'certificate_no', width: 180, ellipsis: { tooltip: true }, render(row) {
...
async function handleReview() {
  ...
  const row = currentVerification.value
  ...
  <n-card :title="t('adminRealname.title')">
    <n-space vertical>
      <n-space align="center">
        <n-text depth="3">
          {{ t('adminRealname.totalRecords', { total }) }}
        </n-text>
        <n-divider vertical />
        <n-text depth="3">
          {{ t('adminRealname.statusFilter') }}
        </n-text>
        <n-select
    ...
    <n-modal v-model:show="showReviewModal" preset="card" :title="t('adminRealname.reviewTitle')" style="width: 500px;">
      ...
      <template #footer>
        <n-space justify="end">
          <NButton @click="showReviewModal = false">
            {{ t('common.cancel') }}
          </NButton>
          <NButton type="primary" :loading="submitting" @click="handleReview">
            {{ t('common.confirm') }}
          </NButton>
        </n-space>
      </template>
    ...
    <n-modal v-model:show="showDetailModal" preset="card" :title="t('realname.detailTitle')" style="width: 600px;">
      <n-descriptions v-if="currentVerification" :column="1" label-placement="left" bordered>
        ...
        <n-descriptions-item :label="t('realname.certificateNo')">
          {{ maskCertificateNo(currentVerification.certificate_no || '') || '-' }}
        </n-descriptions-item>
