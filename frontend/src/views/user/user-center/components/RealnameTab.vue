  ...
  <div class="realname-container">
    ...
    <n-card v-else-if="!hasVerification" :title="t('realname.title')" class="realname-card">
      <n-result status="info" :title="t('realname.notVerifiedTitle')" :description="realnameNotifyText">
        <template #footer>
          <n-button type="primary" @click="showSubmitModal = true">{{ t('realname.verifyNow') }}</n-button>
        </template>
      </n-result>
    </n-card>
    ...
    <n-card v-else-if="verificationStatus === 0" :title="t('realname.title')" class="realname-card">
      <n-result status="warning" :title="t('realname.pendingTitle')" :description="`${t('realname.pendingDescPrefix')}${submittedTime}${t('realname.pendingDescSuffix')}`">
        <template #footer>
          <n-space>
            <n-button @click="showDetailModal = true">{{ t('realname.viewDetail') }}</n-button>
          </n-space>
        </template>
      </n-result>
    </n-card>
    ...
    <n-card v-else-if="verificationStatus === 2" :title="t('realname.title')" class="realname-card">
      <n-result status="error" :title="t('realname.rejectedTitle')" :description="`${t('realname.rejectedReasonPrefix')}${rejectReason || t('realname.rejectedFallbackReason')}`">
        <template #footer>
          <n-space>
            <n-button type="primary" @click="showSubmitModal = true">{{ t('realname.retry') }}</n-button>
          </n-space>
        </template>
      </n-result>
    </n-card>
    ...
    <n-modal v-model:show="showSubmitModal" preset="card" :title="t('realname.submitTitle')" style="width: 520px;">
      ...
      <template #footer>
        <n-space justify="end">
          <n-button @click="showSubmitModal = false">{{ t('realname.cancel') }}</n-button>
          <n-button type="primary" :loading="submitting" @click="handleSubmit">{{ t('realname.submit') }}</n-button>
        </n-space>
      </template>
    </n-modal>
    ...
    <n-modal v-model:show="showDetailModal" preset="card" :title="t('realname.detailTitle')" style="width: 500px;">
      <n-descriptions v-if="realnameInfo" :column="1" label-placement="left" bordered>
        ...
        <n-descriptions-item :label="t('realname.status')">
          <n-tag :type="getStatusType(realnameInfo.status)">{{ getStatusText(realnameInfo.status) }}</n-tag>
        </n-descriptions-item>
        <n-descriptions-item v-if="realnameInfo.status === 2" :label="t('realname.rejectReason')">{{ realnameInfo.rejectReason || '-' }}</n-descriptions-item>
...
// 表单验证
const validateCertificateNo = (_rule: FormItemRule, value: string) => {
  if (!value) return new Error(t('realname.certificateNoRequired'))
  if (form.value.certificate_type === 1) {
    if (!/^\d{17}[\dXx]$/.test(value)) return new Error(t('realname.idCardInvalid'))
  }
  return true
}
