...
| `/system-mgr/settings` | 系统设置（含系统监控与调试能力） | `views/admin/settings/index.vue` |

> 说明：管理端侧边栏已移除独立“调试”菜单与页面，调试能力统一收敛在“系统设置”内。

**管理端路径配置**：
- 环境变量：`VITE_ADMIN_BASE_PATH=/system-mgr`
- 每次打包可以修改，增加安全性
- 只有管理员登录后才会加载这些路由
...
2. **访问方式**：
   ...
   http://localhost:5173/#/system-mgr/settings
   ```
...
**答案**：
- 前端：`frontend/.env` 中的 `VITE_ADMIN_BASE_PATH`
