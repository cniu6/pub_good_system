...
> 
> **关联文件**:
> - `frontend/src/service/http/` - Alova 请求封装
> - `frontend/src/service/api/user/*.ts` - 用户 API
> - `frontend/src/service/api/admin/*.ts` - 管理端 API（`base.ts` 运行时 base）
> - `frontend/src/service/api/app-config.ts` - 公开配置
> - `frontend/src/typings/*.d.ts` - 类型定义

---

## 2026-07 更新：管理端 API 前缀

- 不要写死 `/api/v1/admin`。
- 使用 `getAdminApiBase()` / `adminApiUrl()`（`service/api/admin/base.ts`）。
- 启动时 `settingsStore.loadConfig()` 从 app-config 注入 `admin_api_path`。
- 详见 [管理端路径与Swagger自适应.md](./管理端路径与Swagger自适应.md) 与 [网络请求与API定义.md](../frontend/src/service/网络请求与API定义.md)。
